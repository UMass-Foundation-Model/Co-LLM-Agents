...
from agents import vision_LLM_agent, MCTS_vision_agent
...
if __name__ == '__main__':
    ...
    def MCTS_vision_agent_fn(arena_id, env):
        ...
        return MCTS_vision_agent(**args_mcts)


    def vision_LLM_agent_fn(arena_id, env):
        args_LLM = dict(agent_id=num_agents,
                           char_index=num_agents - 1,
                           args=args,)
        return vision_LLM_agent(**args_LLM)

    

    agents = [vision_LLM_agent_fn]
    if args.use_alice:
        agents = [MCTS_vision_agent_fn] + agents
