...
def train(
    ...
    def tokenize_dialog(datapoint):
        ...
        assert len(dialog_tokens) > cutoff_len, f"too long! {dialog_tokens}"
        ...
        if not train_on_inputs:
            ...
            return {"input_ids": prompt_tokens, "attention_mask": [1] * len(dialog_tokens), "labels": prompt_tokens.copy()}


    # def generate_and_tokenize_prompt(data_point):
        # full_prompt = generate_prompt(data_point)
        # tokenized_full_prompt = tokenize_dialog(data_point)
        # if not train_on_inputs:
    ...
    with accelerator.main_process_first():
        if val_set_size > 0:
            ...
            train_data = (
                train_val["train"].shuffle(seed=42).map(tokenize_dialog)
            )
            val_data = (
                train_val["test"].shuffle(seed=42).map(tokenize_dialog)
            )
        else:
            train_data = data["train"].shuffle(seed=42).map(tokenize_dialog)
            val_data = None
    ...
    for epoch in range(starting_epoch, num_epochs):
        ...
        for step, batch in zip(step_range, train_dataloader):
            ...
            batch = prepare_inputs(batch, accelerator.device)
            outputs = model(**batch)
            loss = outputs.loss
